...
function Alert({
  ...
  return (
    <TextClassContext.Provider value={cn('text-sm text-foreground', variant === 'destructive' && 'text-destructive', className)}>
      <View role="alert" className={cn('relative w-full rounded-lg border border-border bg-card px-4 pb-2 pt-3.5', className)} {...props}>
...
function AlertDescription({ className, ...props }: React.ComponentProps<typeof Text> & React.RefAttributes<Text>) {
  ...
  return <Text className={cn('ml-0.5 pb-1.5 pl-6 text-sm leading-relaxed text-muted-foreground', textClass?.includes('text-destructive') && 'text-destructive/90', className)} {...props} />;
}
