...
function SelectValue({
  ...
  return <SelectPrimitive.Value ref={ref} className={cn('line-clamp-1 flex flex-row items-center gap-2 text-sm text-foreground', !value && 'text-muted-foreground', className)} {...props} />;
}
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      {...props}>
      <>{children}</>
      {showIcon ? <Icon as={ChevronDown} aria-hidden={true} className="size-4 text-muted-foreground" /> : null}
    </SelectPrimitive.Trigger>
  );
}
            ...
            <NativeOnlyAnimatedView className="z-50" entering={FadeIn} exiting={FadeOut}>
              <SelectPrimitive.Content
                className={cn(
                  ...
                  Platform.select({
                    web: cn('origin-(--radix-select-content-transform-origin) max-h-52 overflow-y-auto overflow-x-hidden animate-in fade-in-0 zoom-in-95', props.side === 'bottom' && 'slide-in-from-top-2', props.side === 'top' && 'slide-in-from-bottom-2'),
                    native: 'p-1'
                  }),
                  position === 'popper' &&
                    Platform.select({
...
function SelectLabel({ className, ...props }: SelectPrimitive.LabelProps & React.RefAttributes<SelectPrimitive.LabelRef>) {
  return <SelectPrimitive.Label className={cn('px-2 py-2 text-xs text-muted-foreground sm:py-1.5', className)} {...props} />;
}

function SelectItem({ className, showCheck = true, ...props }: SelectPrimitive.ItemProps & React.RefAttributes<SelectPrimitive.ItemRef> & { showCheck?: boolean }) {
  return (
...
function SelectSeparator({ className, ...props }: SelectPrimitive.SeparatorProps & React.RefAttributes<SelectPrimitive.SeparatorRef>) {
  return <SelectPrimitive.Separator className={cn('-mx-1 my-1 h-px bg-border', Platform.select({ web: 'pointer-events-none' }), className)} {...props} />;
}
...
export { NativeSelectScrollView, type Option, Select, SelectContent, SelectGroup, SelectItem, SelectLabel, SelectScrollDownButton, SelectScrollUpButton, SelectSeparator, SelectTrigger, SelectValue };
